...
export async function getAllArticles(req, res) {
  try {
    const articles = await articleRepository.getAllArticlesWithJournalist();
    res.json(articles);
  } catch (error) {
...
export async function getArticleById(req, res) {
  try {
    const article = await articleRepository.getArticlesWithJournalistById(req.params.id);
    if (!article) {
...
export async function getArticlesByJournalistId (req,res) {
  try{
    ...
    console.error("Error fecthing article by journalist id ",error)
    res.status(500).json({ message: "Server error" });
  }
}
